...
namespace Consul.MasterSlaverSwitching.Slaver
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var serviceId = pro.SelfRegisterService(serviceName, new TimeSpan(0, 0, 0, 10), new string[] { MasterOrSlaver.Slaver.ToString(), serviceZone }).ConfigureAwait(false).GetAwaiter().GetResult();
            MainEntry demo = new MainEntry();
            SwitchingProvider sp = new SwitchingProvider(consul, demo.Run, demo.Stop, serviceName, serviceId, serviceZone, MasterOrSlaver.Slaver, LogMsg, new TimeSpan(0, 0, 10), 3);
            sp.Begin();
